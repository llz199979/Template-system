...
const attendence = {
  ...
  mutations: {
    ...
    set_getTemp:(state,data) =>state.temp = data,
  },
  actions: {
    /**
     * 考勤管理
    */
    ...
    downloadExcel({state}, page) {
      return new Promise((resolve, reject) => {
        let url = `http://47.106.181.248:8080/getEquipmentExport`;
    ...
    getDownloadExcel({state}, page) {
      return new Promise((resolve, reject) => {
        ...
        str = str.slice(1,str.length-1)
        let url = `http://47.106.181.248:8080/getCheckingInPage2Export?${str}`
    ...
    // /getTemporaryPicture 查询临时数据
    getTemp({state, commit}, page) {
      let url = `/getTemporaryPicture?time=${page.time}&type=${page.type}&cpuId=${page.cpuId}` ;
      fx.setConnect({url})
      .then(res => {
        commit('set_getTemp', res.data.data);
        page.total = res.data.data.total * 1;
      }).catch(err => {
